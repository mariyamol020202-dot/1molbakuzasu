...
function ensureCloudStyles() {
  ...
  link.rel = 'stylesheet';
  link.href = 'css/family-cloud.css?v=20260820-1';
    ...
    form.onsubmit = async (event) => {
      ...
      try {
        ...
        if (!sessionData?.session?.user) {
          const { error: signError } = await supabaseClient.auth.signInAnonymously();
        ...
        console.error(error);
        status.textContent = 'Не получилось войти. Проверьте интернет и попробуйте ещё раз.';
      } finally {
        button.disabled = false;
      }
    };
...
function ensureFamilyBoardSection() {
  if (!/about\.html$/i.test(location.pathname) && !document.querySelector('.family-story-section')) return null;
  ...
  section.innerHTML = `
    <div class="family-board-head">
      <div><p class="eyebrow">FAMILY BOARD</p><h2>Оставьте что-нибудь<br>на семейной доске.</h2></div>
      <div class="family-board-actions"><p>Напоминание, шутку, поздравление или просто мысль.</p><button class="cloud-add-button light" id="addFamilyNote" type="button">＋ Написать мелом</button></div>
    </div>
    <div class="chalkboard"><div class="chalk-notes" id="familyNotesGrid"><div class="chalk-loading">Загружаем записки…</div></div></div>`;

  const anchor = document.querySelector('.branches-section') || document.querySelector('.principles-section') || document.querySelector('footer');
